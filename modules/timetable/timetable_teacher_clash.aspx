<%@ Page Language="VB" AutoEventWireup="false" CodeFile="timetable_teacher_clash.aspx.vb" Inherits="modules_timetable_timetable_teacher_clash" %>
        ...
        <table width="880"  border="0" id="TABLE1" onclick="return TABLE1_onclick()">
            <tr>
                <td colspan="2">
                    <table width="100%">
                        <tr>
                            <td width="400"><asp:Label ID="Label9" runat="server" Font-Bold="True" Font-Size="Small" Text=":: TEACHER CLASHES"></asp:Label></td>
          ...
          <tr>
            ...
            <td>
                <asp:GridView ID="gvTeachers" runat="server" AutoGenerateColumns="False" 
                    DataSourceID="dsSqlTeachers" Width="100%" 
                        EmptyDataText="No teacher clashes to display." 
                    ...
                    <asp:TemplateField HeaderText="Periods Taught">
                        <ItemTemplate>
                            <asp:Label ID="Label11" runat="server" Font-Bold="True" 
                                Text='<%# CType(Databinder.Eval(Container.DataItem, "TeacherName"), String).ToUpper & ": This teacher is currently required to deliver " & CType(Databinder.Eval(Container.DataItem, "PeriodCount"), String) & " lessons at the same time on " & CType(Databinder.Eval(Container.DataItem, "DayOfWeekName"), String) & ", Period " & CType(Databinder.Eval(Container.DataItem, "PeriodNumber"), String) & "." %>'></asp:Label>
                            <br />&nbsp;
                            <table width="100%">
                                <tr>
                                    <td width="30">
                            ...
                            <asp:SqlDataSource ID="dsSqlGetTeacherLoad" runat="server" 
                                ConnectionString="<%$ ConnectionStrings:iTecon %>" 
                                SelectCommand="SELECT [PeriodId], e.FirstName + ' ' + e.Surname as TeacherName, [DayOfWeek], dbo.FUNC_GET_WEEKDAYNAME([DayOfWeek]) as DayOfWeekName, [PeriodNumber], d.VenueName, b.ClassName, c.SubjectName FROM [dbo].[Core_TimetablePeriod] a INNER JOIN dbo.TEMPClass b ON a.ClassId  = b.ClassId INNER JOIN dbo.TEMPSubject c ON a.SubjectId = c.SubjectId INNER JOIN dbo.Core_TuitionVenue d ON a.RoomId = d.VenueId INNER JOIN dbo.Core_User e ON a.Remark1 = e.Username WHERE a.Remark1 = @Username AND dbo.FUNC_GET_WEEKDAYNAME([DayOfWeek]) = @DayOfWeekName AND PeriodNumber = @PeriodNumber ORDER BY TeacherName, SubjectName, VenueName ">
                                <SelectParameters>
                                    <asp:QueryStringParameter Name="Username" QueryStringField="a" DbType="String" />
                ...
                <EditRowStyle CssClass="defaultFormElementStyle" />
                    ...
                    SelectCommand="SELECT b.Username, b.FirstName + ' ' + b.Surname as TeacherName, dbo.FUNC_GET_WEEKDAYNAME([DayOfWeek]) as DayOfWeekName, PeriodNumber, COUNT(Username) AS PeriodCount FROM dbo.Core_TimetablePeriod a INNER JOIN dbo.Core_User b ON a.Remark1 = b.Username WHERE a.Remark1 = @Username GROUP BY b.Username, b.FirstName + ' ' + b.Surname, dbo.FUNC_GET_WEEKDAYNAME([DayOfWeek]), PeriodNumber HAVING Count(Username) > 1">
                        <SelectParameters>
