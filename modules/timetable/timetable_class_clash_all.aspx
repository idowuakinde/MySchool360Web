<%@ Page Language="VB" AutoEventWireup="false" CodeFile="timetable_class_clash_all.aspx.vb" Inherits="modules_timetable_timetable_class_clash_all" %>
        ...
        <table width="880"  border="0" id="TABLE1" onclick="return TABLE1_onclick()">
            <tr>
                <td colspan="2">
                    <table width="100%">
                        <tr>
                            <td width="700"><asp:Label ID="Label9" runat="server" Font-Bold="True" Font-Size="Small" Text=":: CLASS CLASHES"></asp:Label></td>
          ...
          <tr>
            ...
            <td>
                <asp:GridView ID="gvClasses" runat="server" AutoGenerateColumns="False" 
                    DataSourceID="dsSqlClasses" Width="100%" 
                        EmptyDataText="No class clashes to display." 
                    ...
                    <asp:TemplateField HeaderText="Periods">
                        <ItemTemplate>
                            <asp:Label ID="Label11" runat="server" Font-Bold="True" 
                                Text='<%# CType(Databinder.Eval(Container.DataItem, "ClassName"), String).ToUpper & ": This class has " & CType(Databinder.Eval(Container.DataItem, "PeriodCount"), String) & " clashing lessons on " & CType(Databinder.Eval(Container.DataItem, "DayOfWeekName"), String) & ", Period " & CType(Databinder.Eval(Container.DataItem, "PeriodNumber"), String) & "." %>'></asp:Label>
                            <br />&nbsp;
                            <table width="100%">
                                <tr>
                                    <td width="30">
                                        &nbsp;</td>
                                    <td>
                                        <asp:GridView ID="gvClassLoad" runat="server" AutoGenerateColumns="False" 
                                            ...
                                            OnRowDataBound="InnerGrid_RowDataBound" OnDataBound="InnerGrid_DataBound">
                            ...
                            <asp:SqlDataSource ID="dsSqlGetClassLoad" runat="server" 
                                ConnectionString="<%$ ConnectionStrings:iTecon %>" 
                                SelectCommand="SELECT [PeriodId], b.ClassName, [DayOfWeek], dbo.FUNC_GET_WEEKDAYNAME([DayOfWeek]) as DayOfWeekName, [PeriodNumber], d.VenueName, c.SubjectName, e.FirstName + ' ' + e.Surname as TeacherName FROM [dbo].[Core_TimetablePeriod] a INNER JOIN dbo.TEMPClass b ON a.ClassId  = b.ClassId INNER JOIN dbo.TEMPSubject c ON a.SubjectId = c.SubjectId INNER JOIN dbo.Core_TuitionVenue d ON a.RoomId = d.VenueId INNER JOIN dbo.Core_User e ON a.Remark1 = e.Username WHERE b.ClassName = @ClassName AND dbo.FUNC_GET_WEEKDAYNAME([DayOfWeek]) = @DayOfWeekName AND PeriodNumber = @PeriodNumber ORDER BY ClassName, VenueName, SubjectName, TeacherName  ">
                                <SelectParameters>
                ...
                <EditRowStyle CssClass="defaultFormElementStyle" />
                    ...
                    SelectCommand="SELECT b.ClassName, dbo.FUNC_GET_WEEKDAYNAME([DayOfWeek]) as DayOfWeekName, PeriodNumber, COUNT(ClassName) AS PeriodCount FROM dbo.Core_TimetablePeriod a INNER JOIN dbo.TEMPClass b ON a.ClassId = b.ClassId GROUP BY b.ClassName, dbo.FUNC_GET_WEEKDAYNAME([DayOfWeek]), PeriodNumber HAVING Count(ClassName) &gt; 1 ">
                    </asp:SqlDataSource>
                </td>
          </tr>
          <tr>
