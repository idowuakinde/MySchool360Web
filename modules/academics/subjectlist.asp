...
<%
	...
	studentClass = GetStudentClass(request.QueryString("b"))
%>
<%
	Dim rstSubjects
	...
	rstSubjects.Source = "SELECT * FROM dbo.TEMPSubject where ShortName IN (SELECT SubjectShortName FROM dbo.TEMPStudentSubject WHERE PortalNumber = '" & matricNumber & "') "
...
<table width="100%"  border="0">
  <tr>
    <td width="70%" valign="top"> <span class="pageHeaderStyleAMMA">&nbsp;:: Lesson Notes for <%= studentClass %> </span></td>
  </tr>
...
%>
              <li>
			    <strong>
				<a href="mysyllabus.aspx?a=<%= matricNumber %>&amp;b=<%= studentClass %>&amp;c=<%= rstSubjects("ShortName") %>&amp;d=<%= rstSubjects("SubjectId") %>"><%= rstSubjects("SubjectName") %></a>
				</strong>
				<br />&nbsp;
			  </li>
              <% 
...
<%
    ...
    Function GetStudentClass(literalClass)
        dim ret

        if instr(1, literalClass, ".") = 0 then
            ret = literalClass
        else
            ret = left(literalClass, instr(1, literalClass, ".")-1)
        end if
