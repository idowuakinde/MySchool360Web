...
Partial Class modules_data_print_student_result_primary_CA4
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Me.dsSqlStudents.SelectCommand = " SELECT CustomerId, RefCode, DistrictCode as ClassName, CustomerCode as PortalNumber, FirstName, LastName, SyncStatus as Gender, Activated as Boarder, ISNULL(Score_PE, '') AS Score_PE, ISNULL(ClubsAndExtras, '') AS ClubsAndExtras, ISNULL(DaysAbsent, 0) as DaysAbsent, CASE ISNULL(Punctuality, 0) WHEN 0 THEN 'Unspecified' WHEN 1 THEN 'Never punctual' WHEN 2 THEN 'Sometimes punctual' WHEN 3 THEN 'Always punctual' END AS Punctuality, REPLACE(ISNULL(ClassTeacherComment, ''), '''''', '''') AS ClassTeacherComment, CASE ISNULL(Remark1, '0') WHEN '0' THEN 'Unspecified' WHEN 'A' THEN 'Very Good' WHEN 'B' THEN 'Average' WHEN 'C' THEN 'Poor' END AS Handwriting FROM dbo.TEMPCustomers_MultiSession a LEFT JOIN dbo.TEMPStudentSubjectScoreTeacherComment2 b ON a.CustomerCode = b.PortalNumber AND SessionName = '" & sessionName & "' AND TermName = '" & TermName & "' AND AssessmentType = '" & assessmentType & "' WHERE CustomerCode = '" & portalNumber & "' ORDER BY PortalNumber "
        Me.dsSqlStudents.SelectParameters(0).DefaultValue = className
        Me.dsSqlStudents.DataBind()

        Me.dsSqlSubjects.SelectCommand = " SELECT CustomerId, CustomerCode as PortalNumber, d.ShortName AS SubjectShortName, d.SubjectName, ScoreId, ISNULL((SELECT ISNULL(Score, 0) + ISNULL(ScoreClassTest, 0) FROM dbo.TEMPStudentSubjectScore WHERE PortalNumber = c.PortalNumber AND SubjectShortName = d.ShortName AND SessionName = c.SessionName AND TermName = c.TermName AND AssessmentType = 'Test'), 0) AS MidTermSubjectScore, ISNULL(c.Score, 0) + ISNULL(c.ScoreClassTest, 0) AS TermEndSubjectScore FROM dbo.TEMPCustomers_MultiSession a INNER JOIN dbo.TEMPStudentSubject b ON a.CustomerCode = b.PortalNumber LEFT JOIN dbo.TEMPStudentSubjectScore2 c ON b.PortalNumber = c.PortalNumber AND b.SubjectShortName = c.SubjectShortName AND b.SessionName = c.SessionName AND b.TermName = c.TermName AND AssessmentType = @AssessmentType INNER JOIN dbo.TEMPSubject d ON b.SubjectShortName = d.ShortName WHERE a.CustomerCode = @PortalNumber AND ISNULL(c.Score, 0) + ISNULL(c.ScoreClassTest, 0) >= 0 AND IsNull(a.BelongToCode, '2012/2013') = '" & sessionName & "' AND b.SessionName = @SessionName AND b.TermName = @TermName ORDER BY StudentSubjectId "
