...
Partial Class modules_data_comments_classteacher_primary_CA6
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Me.dsSqlStudents.SelectCommand = " SELECT CustomerId, RefCode, DistrictCode as ClassName, CustomerCode as PortalNumber, FirstName, LastName, SyncStatus as Gender, Activated as Boarder, ISNULL(Score_PE, '') AS Score_PE, ISNULL(ClubsAndExtras, '') AS ClubsAndExtras, ISNULL(DaysAbsent, 0) as DaysAbsent, ISNULL(Punctuality, 0) AS Punctuality, ISNULL(ClassTeacherComment, '') AS ClassTeacherComment, IsNull(Remark1, 0) AS Handwriting FROM dbo.TEMPCustomers a LEFT JOIN dbo.TEMPStudentSubjectScoreTeacherComment2 b ON a.CustomerCode = b.PortalNumber AND SessionName = '" & sessionName & "' AND TermName = '" & TermName & "' AND AssessmentType = '" & assessmentType & "' WHERE DistrictCode = '" & className & "' ORDER BY PortalNumber "
