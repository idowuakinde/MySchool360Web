<%@ Page Language="VB" AutoEventWireup="false" CodeFile="approve_primary_CA2.aspx.vb" Inherits="modules_data_approve_primary_CA2" %>
        ...
        <table border="0" width="100%">
            <tr>
                <td>
                        ...
                        Text=":: Review Students' Scores for End of Term"></asp:Label></td>
            </tr>
            <tr>
                <td>
...
&nbsp;&nbsp;<asp:Button ID="btnEmailBatch" runat="server" 
        ...
        <asp:GridView ID="gvMaster" runat="server" CssClass="noBorderedMenuTable" AutoGenerateColumns="False" 
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="C.A." SortExpression="Score">
                    <ItemTemplate>
                        <asp:Label ID="lblMidTermCA" runat="server" Text='<%# "Before Mid-Term: " & Common.GetScoreMask(Common.GetMidTermScore(CType(Databinder.Eval(Container.DataItem, "PortalNumber"), String), CType(Databinder.Eval(Container.DataItem, "ClassName"), String), subjectShortName, sessionName, termName, "CA"), CType(Databinder.Eval(Container.DataItem, "ClassName"), String)) %>'></asp:Label>
                        <br />
                        <asp:Label ID="Label6" runat="server" Text='<%# "After Mid-Term: " & CType(Databinder.Eval(Container.DataItem, "Score"), Int32) %>'></asp:Label>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Center" />
                    <ItemStyle HorizontalAlign="Center" />
                </asp:TemplateField>
                <asp:TemplateField HeaderText="Exam" SortExpression="ScoreClassTest">
                    <ItemTemplate>
                        <asp:Label ID="lblMidTermCycleTest" runat="server" Text='<%# "Before Mid-Term: " & Common.GetScoreMask(Common.GetMidTermScore(CType(Databinder.Eval(Container.DataItem, "PortalNumber"), String), CType(Databinder.Eval(Container.DataItem, "ClassName"), String), subjectShortName, sessionName, termName, "CYCLETEST"), CType(Databinder.Eval(Container.DataItem, "ClassName"), String)) %>'></asp:Label>
                        <br />
                        <asp:Label ID="Label7" runat="server" Text='<%# "After Mid-Term: " & CType(Databinder.Eval(Container.DataItem, "ScoreClassTest"), Int32) %>'></asp:Label>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Center" />
                    <ItemStyle HorizontalAlign="Center" />
                </asp:TemplateField>
            ...
            <AlternatingRowStyle BackColor="White" />
                    ...
                    <asp:SqlDataSource ID="dsSqlStudents" runat="server" ConnectionString="<%$ ConnectionStrings:iTecon %>"                        
                        SelectCommand=" SELECT CustomerId, ISNULL(c.ScoreId, 0) AS ScoreId, RefCode, DistrictCode as ClassName, CustomerCode as PortalNumber, FirstName, LastName, SyncStatus as Gender, Activated as Boarder, b.SubjectShortName, ISNULL(c.Score, 0) AS Score, ISNULL(c.ScoreClassTest, 0) AS ScoreClassTest, ISNULL(c.Comment, '') as Comment, ISNULL(c.Remark1, '') AS ApprovalStatus FROM dbo.TEMPCustomers a INNER JOIN dbo.TEMPStudentSubject b ON a.CustomerCode = b.PortalNumber LEFT JOIN dbo.TEMPStudentSubjectScore2 c ON b.PortalNumber = c.PortalNumber AND b.SubjectShortName = c.SubjectShortName AND b.SessionName = c.SessionName AND b.TermName = c.TermName WHERE a.DistrictCode = @ClassName AND b.SubjectShortName = @SubjectShortName ORDER BY CustomerCode ">
            <SelectParameters>
