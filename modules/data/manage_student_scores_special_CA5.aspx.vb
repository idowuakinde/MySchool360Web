...
Partial Class modules_data_manage_student_scores_special_CA5
    ...
    Public TermName As String = "3rd Term"
    ...
    Protected Sub btnSaveScores_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveScores.Click
        ...
        For i As Int32 = 0 To Me.gvMaster.Rows.Count - 1
            ...
            Dim subjectKnowledge As Double = CType(CType(Me.gvMaster.Rows(i).FindControl("ddlTestScore"), DropDownList).Text, Double)
            Dim subjectInvolvement As Double = CType(CType(Me.gvMaster.Rows(i).FindControl("ddlClassTests"), DropDownList).Text, Double)
            Dim subjectComment As String = CType(Me.gvMaster.Rows(i).FindControl("txtAddSubjectComment"), TextBox).Text.Replace("'", "''")
            Dim total As Double = subjectKnowledge + subjectInvolvement : CType(Me.gvMaster.Rows(i).FindControl("lblTotal"), Label).Text = total
            ...
            sqlInsert &= "      SET Score = " & subjectKnowledge & ", "
            sqlInsert &= "          ScoreClassTest = " & subjectInvolvement & ", "
            ...
            sqlInsert &= "ELSE "
            sqlInsert &= "      INSERT INTO dbo.TEMPStudentSubjectScore(PortalNumber, SubjectShortName, SessionName, TermName, AssessmentType, Score, ScoreClassTest, Comment) "
            sqlInsert &= "      VALUES('" & portalNumber & "','" & subjectShortName & "','" & sessionName & "','" & TermName & "','" & assessmentType & "'," & subjectKnowledge & "," & subjectInvolvement & ",'" & subjectComment.Replace("'", "''") & "') "
