...
Partial Class modules_data_print_student_result_sec_CA1
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Me.dsSqlStudents.SelectCommand = " SELECT CustomerId, RefCode, DistrictCode as ClassName, CustomerCode as PortalNumber, FirstName, LastName, SyncStatus as Gender, Activated as Boarder, ISNULL(ClassTeacherComment, '') AS ClassTeacherComment, ISNULL(Remark1, 0) AS StudentAverage, ISNULL(Remark2, 0) AS ClassAverage FROM dbo.TEMPCustomers_MultiSession a LEFT JOIN dbo.TEMPStudentSubjectScoreTeacherComment b ON a.CustomerCode = b.PortalNumber AND SessionName = '" & sessionName & "' AND TermName = '" & TermName & "' AND AssessmentType = '" & assessmentType & "' WHERE CustomerCode = '" & portalNumber & "' ORDER BY PortalNumber "
        Me.dsSqlStudents.SelectParameters(0).DefaultValue = className
        Me.dsSqlStudents.DataBind()

        Me.dsSqlSubjects.SelectCommand = " SELECT CustomerId, CustomerCode as PortalNumber, DistrictCode as ClassName, d.SubjectName, ScoreId, ISNULL(c.Score, 0) AS CycleTest, ISNULL(c.ScoreClassTest, 0) AS ClassTests, ISNULL(c.ScoreAssignments, 0) AS Assignments, ISNULL(c.ScoreClassWork, 0) AS ClassWork, ISNULL(c.Score, 0) + ISNULL(c.ScoreClassTest, 0) + ISNULL(c.ScoreAssignments, 0) + ISNULL(c.ScoreClassWork, 0) AS MidTermSubjectScore, ISNULL(c.Comment, '') AS SubjectComment FROM dbo.TEMPCustomers_MultiSession a INNER JOIN dbo.TEMPStudentSubject b ON a.CustomerCode = b.PortalNumber LEFT JOIN dbo.TEMPStudentSubjectScore c ON b.PortalNumber = c.PortalNumber AND b.SubjectShortName = c.SubjectShortName AND b.SessionName = c.SessionName AND b.TermName = c.TermName AND AssessmentType = @AssessmentType INNER JOIN dbo.TEMPSubject d ON b.SubjectShortName = d.ShortName WHERE a.CustomerCode = @PortalNumber AND IsNull(a.BelongToCode, '2012/2013') = '" & sessionName & "' AND b.SessionName = @SessionName AND b.TermName = @TermName ORDER BY StudentSubjectId "
