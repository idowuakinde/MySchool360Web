...
Partial Class modules_reports_school_summary_businesstypes
    Inherits System.Web.UI.Page

    Public serialNumber As Int32, totalBusinessTypeCount As Int32
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Chart.DefaultSeries.DefaultElement.ToolTip = "N %yvalue collected from %xvalue students"
        Chart.Debug = True
        Chart.Mentor = False
        ...
        de.SqlStatement = "select ZoneCode, sum(amount) as Revenue from dbo.TEMPOfficeEntries " & filterExpression & " group by ZoneCode order by ZoneCode"
        ...
        Chart1.DefaultSeries.DefaultElement.ToolTip = "N %yvalue collected from %xvalue students"
        Chart1.Debug = True
        Chart1.Mentor = False
    ...
    Protected Sub gvUsers_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvUsers.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            totalBusinessTypeCount += rowView("BusinessTypeCount").ToString
            total += rowView("Revenue").ToString
        ElseIf e.Row.RowType = DataControlRowType.Footer Then
            Me.Label4.Text = "TOTAL REVENUE COLLECTED (ACROSS " & serialNumber & " LEVELS) BETWEEN " & MonthName(filterDate_Start.Month, True) & " " & filterDate_Start.Day & ", " & filterDate_Start.Year & " AND " & MonthName(filterDate_End.Month, True) & " " & filterDate_End.Day & ", " & filterDate_End.Year & ": N " & FormatNumber(total, 2)
        End If
    End Sub

    Private Sub SetGridSource(ByVal filterExpression As String)
        Me.dsSqlRevenueHeadSummaries.SelectCommand = "select UPPER(ZoneCode) as BusinessType, count(ZoneCode) as BusinessTypeCount, sum(amount) as Revenue from dbo.TEMPOfficeEntries " & filterExpression & " group by ZoneCode order by sum(amount) desc"
    End Sub
