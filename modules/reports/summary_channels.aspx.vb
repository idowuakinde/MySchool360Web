...
Partial Class modules_reports_school_summary_channels
    Inherits System.Web.UI.Page

    Public serialNumber As Int32, totalChannelCount As Int32
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Chart.DefaultSeries.DefaultElement.ToolTip = "N %yvalue collected via %xvalue"
        Chart.Debug = True
        Chart.Mentor = False
        ...
        de.SqlStatement = "select ModeOfPayment, sum(amount) as Revenue from dbo.TEMPOfficeEntries " & filterExpression & " group by ModeOfPayment order by sum(amount) desc"
        ...
        Chart1.DefaultSeries.DefaultElement.ToolTip = "N %yvalue collected via %xvalue"
        Chart1.Debug = True
        Chart1.Mentor = False
    ...
    Protected Sub gvUsers_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvUsers.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            totalChannelCount += rowView("ChannelCount").ToString
            total += rowView("Revenue").ToString
        ElseIf e.Row.RowType = DataControlRowType.Footer Then
            Me.Label4.Text = "TOTAL REVENUE COLLECTED (ACROSS " & serialNumber & " CHANNELS) BETWEEN " & MonthName(filterDate_Start.Month, True) & " " & filterDate_Start.Day & ", " & filterDate_Start.Year & " AND " & MonthName(filterDate_End.Month, True) & " " & filterDate_End.Day & ", " & filterDate_End.Year & ": N " & FormatNumber(total, 2)
        End If
    End Sub

    Private Sub SetGridSource(ByVal filterExpression As String)
        Me.dsSqlRevenueHeadSummaries.SelectCommand = "select UPPER(ModeOfPayment) as Channel, count(ModeOfPayment) as ChannelCount, sum(amount) as Revenue from dbo.TEMPOfficeEntries " & filterExpression & " group by ModeOfPayment order by sum(amount) desc"
    End Sub
