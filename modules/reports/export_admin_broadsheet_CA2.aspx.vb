...
Partial Class modules_reports_export_admin_broadsheet_CA2
    Inherits System.Web.UI.Page

    Dim portalNumber As String = "", assessmentType As String = "Test2"
    Dim className As String, personalTotal As Double, personalAverage As Double, personalAverageTotal As Double, subjectsOfferedByStudent As Int32, subjectTotal() As Double, studentsOfferingSubject() As Int32, subjectAverage As Double, subjectAverageTotal As Double, averageOfAverage As Double, subjectScore As Double, classPosition As Int32
    ...
    Private Sub GenerateWorkSheet(ByVal workBook As GemBox.Spreadsheet.ExcelFile)
        ...
        Select Case className.ToUpper
            Case "YEAR 7", "YEAR 8", "YEAR 9", "YEAR 10", "YEAR 11"     'secondary
                rowsSubjects = Common.GetTableRows(" SELECT DISTINCT a.SubjectId, a.SubjectName, a.ShortName FROM dbo.TEMPSubject a INNER JOIN dbo.TEMPStudentSubject b ON a.ShortName = b.SubjectShortName INNER JOIN dbo.TEMPCustomers c ON b.PortalNumber = c.CustomerCode INNER JOIN dbo.TEMPStudentSubjectScore2 d ON b.PortalNumber = d.PortalNumber AND b.SessionName = d.SessionName AND b.TermName = d.TermName WHERE b.SessionName = '" & sessionName & "' AND b.TermName = '" & termName & "' AND b.PortalNumber IN (SELECT CustomerCode FROM dbo.TEMPCustomers WHERE DistrictCode = '" & className & "') ")
        ...
        End Select
        ReDim subjectTotal(rowsSubjects.Count - 1)
        ReDim studentsOfferingSubject(rowsSubjects.Count - 1)
        ...
        mergedRange.Value = "CHILDVILLE SCHOOLS, LAGOS" & vbCrLf & "BROADSHEET" & vbCrLf & sessionName.ToUpper & " SESSION, " & termName.ToUpper & ", AFTER MID-TERM"
        ...
        For i As Int32 = 0 To rowsStudents.Count - 1
            ...
            For j As Int32 = 0 To rowsSubjects.Count - 1
                subjectScore = Common.GetStudentTermTotalScore(portalNumber, className, CType(rowsSubjects(j).Item("ShortName"), String), sessionName, termName)
                ws.Cells(4 + i, currentColumn + j + 1).Value = FormatNumber(subjectScore, 0)
                ws.Cells(4 + i, currentColumn + j + 1).Style = centeredTextStyle
                personalTotal += FormatNumber(subjectScore, 0)
                subjectTotal(j) += FormatNumber(subjectScore, 0)
                If subjectScore <> 0 Then
                    studentsOfferingSubject(j) += 1
                    subjectsOfferedByStudent += 1
                End If
            Next
            'personalAverage = Common.GetStudentsPersonalTermTotalAverage(portalNumber, className, sessionName, termName)
            personalAverage = FormatNumber(personalTotal / subjectsOfferedByStudent, 1)
            personalAverageTotal += personalAverage
            classPosition = Common.GetClassTermTotalPosition(Common.GetStudentsPersonalTermTotalAverage(portalNumber, className, sessionName, termName), className, sessionName, termName)
        ...
        For i As Int32 = 0 To rowsSubjects.Count - 1
            'ws.Cells(4 + rowsStudents.Count + 1, 1 + (i + 1)).Value = Common.GetClassTermTotalSubjectAverage(className, CType(rowsSubjects(i).Item("ShortName"), String), sessionName, termName)
            subjectAverage = FormatNumber(subjectTotal(i) / studentsOfferingSubject(i), 1)
            subjectAverageTotal += subjectAverage
            ws.Cells(4 + rowsStudents.Count + 1, 1 + (i + 1)).Value = subjectAverage
            ws.Cells(4 + rowsStudents.Count + 1, 1 + (i + 1)).Style = centeredTextStyle
        Next

        'Generate the Class Average row
        'averageOfAverage = FormatNumber(personalAverageTotal / rowsStudents.Count, 1)
        averageOfAverage = Common.GetClassTermTotalAverage(className, sessionName, termName)
        ws.Cells(4 + rowsStudents.Count + 1, 20).Value = FormatNumber(averageOfAverage, 1) : ws.Cells(4 + rowsStudents.Count + 1, 20).Style = centeredTextStyle          ''''commenting this line out because, even though it produces a "fairly correct" result, it results in an accumulation of approximations, thereby causing confusion when compared with the "absolute" Class Average as calculated directly a few lines below...
        ...
        ws.Cells(4 + rowsStudents.Count + 3, 2).Value = FormatNumber(averageOfAverage, 1)          'FormatNumber(subjectAverageTotal / rowsSubjects.Count, 1)       ' Common.GetClassTermTotalAverage(className, sessionName, termName) : ws.Cells(4 + rowsStudents.Count + 3, 2).Style = centeredTextStyle
    ...
    Private Sub DownloadFile(ByVal virtualPath As String)
        ...
        Response.AddHeader("content-disposition", "attachment; filename=ExportBroadSheet_" & sessionName.Replace("/", "") & "Session_" & termName.Replace(" ", "") & "_AfterMidTerm_" & className.Replace(" ", "").Replace("-", "") & ".xlsx")
